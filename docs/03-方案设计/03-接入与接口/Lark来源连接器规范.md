...
连接器直接调用 Lark OpenAPI，并始终使用当前用户身份。AIW 在本机 `~/.aiw/config.yaml` 中保存 App ID、域名和本地回调地址；App Secret 只保存到 macOS Keychain。每次读取都启动新的 OAuth 授权，用户访问令牌和刷新令牌不落盘，命令结束即丢弃。
