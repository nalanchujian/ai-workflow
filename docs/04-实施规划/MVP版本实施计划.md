...
> 历史实施记录：本文记录初始 MVP 的分步实现，不是当前 CLI、文件结构或验收覆盖的权威来源。文中的 `configured:superpowers`、初始文件清单与 AC-1 至 AC-25 均是当时的实现基线；当前内置方法、命令与完整 AC-1 至 AC-28 以《[技能包规范](../03-方案设计/02-核心规范/技能包规范.md)》、《[CLI命令参考](../03-方案设计/03-接入与接口/CLI命令参考.md)》和《[MVP需求与验收规范](../02-需求定义/MVP需求与验收规范.md)》为准。
