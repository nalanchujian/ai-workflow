...
> 历史实施记录：本文记录初始 MVP 的分步实现，代码片段中的 `configured:superpowers` 是当时的兼容契约，不是新用户流程。当前内置方法依赖以《[内置方法依赖设计](../superpowers/specs/2026-08-13-内置方法依赖设计.md)》和《[技能包规范](../03-方案设计/02-核心规范/技能包规范.md)》为准。
