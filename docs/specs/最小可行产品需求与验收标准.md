...
交付可在开发者本机运行的 `aiw` CLI。它能安装并锁定声明式团队技能及其上游方法论引用、建立包含来源快照的七阶段任务，并通过可验证的 Codex Adapter 预演或执行节点运行。
...
- Runner 根据七阶段规则生成 `context-manifest.json` 和 `context.md`，仅包含被允许且已确认的任务文件、阶段技能、锁定的方法论来源和用户任务。
