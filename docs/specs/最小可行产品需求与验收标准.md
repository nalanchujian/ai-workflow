...
- `aiw task init <task-id> --project <path> --source <file-or-url>` 创建 `.aiw/config.yaml`（首次）、`.aiw/tasks/<task-id>/`、`task.yaml`、`task.md` 与首个来源快照。
...
- 快照元数据记录来源、获取时间、内容 SHA-256 与提取器版本；不得记录本机绝对路径、Cookie、令牌或授权头。
