...
- 每个非 `intake` 节点启动前，必须重新核验 `task.yaml.sources[*].contentSha256`、对应 `meta.json.contentSha256` 与 `snapshot.md` 当前字节的 SHA-256，同时核验来源 ID、类型、地址、revision 和章节标识；任一不一致时必须拒绝运行，并要求使用 `task source refresh` 重新固化来源。
...
- 执行模式必须在 Codex 启动前拒绝业务工作树中的未提交变更，并记录 Git 提交与分支基线；执行后采集全部 Git 变更路径、未跟踪文件补丁及 Git 状态。业务代码和测试可按当前节点目标自由修改，不以计划中的文件路径白名单阻断；但 `.aiw/` 只能写入当前任务、当前运行和当前节点声明的产物，写入其他任务事实、修改 Git 历史或切换分支必须写入运行证据、将当前节点标记失败且不得解锁下游节点。
...
- `clarify` 必须生成 `artifacts/decision-register.yaml`：每个无法由现有事实确定、且影响验收或实施范围的事项必须包含一至两个本期继续方案、取舍、AI 推荐和业务化的“需要确认什么、当前情况、不确认的影响”。一次人工选择只能解决一个独立业务结论，且 `affects.acceptanceRefs` 必须恰好关联一个 AC；页面、导出、邮件等可分别确认的事项必须拆成多个决策项。存在未处理事项时，普通 `task approve ... clarify` 必须拒绝；`aiw task review <task-id>` 先展示唯一关联验收项，再选择“本期继续 / 等待外部条件”；选择继续后才能选择 AI 方案或输入人工结论。选择等待时，`review` 记录外部等待并仅阻塞关联工作单元；需求拆期应刷新来源后重新澄清，风险接受只在测试阶段处理。澄清确认后不允许修改决策结论；外部条件满足后仅允许通过 `aiw task decision resolve` 处理：若只恢复执行时机可解锁关联单元，若新增事实影响方案、计划或验收，必须记录结构化事实并使 solution、plan 和下游重新规划。
...
| AC-27 | AI 提出决策并由人工处理 | 澄清产出包含选项、取舍和推荐的决策登记；选择、外部等待、拆期或豁免均形成不可变事实。外部等待解除必须区分仅恢复执行与重新规划；后者记录新增事实，并使 solution、plan 和下游当前结果失效。 |
