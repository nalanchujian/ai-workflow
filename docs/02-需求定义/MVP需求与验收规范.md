...
- `clarify` 必须生成 `artifacts/decision-register.yaml`：每个无法由现有事实确定、且影响验收或实施范围的事项必须包含一至两个本期继续方案、取舍、AI 推荐和业务化的“需要确认什么、当前情况、不确认的影响”。一次人工选择只能解决一个独立业务结论，且 `affects.acceptanceRefs` 必须恰好关联一个 AC；页面、导出、邮件等可分别确认的事项必须拆成多个决策项。存在未处理事项时，普通 `task approve ... clarify` 必须拒绝；`aiw task review <task-id>` 先展示唯一关联验收项，再选择“本期继续 / 等待外部条件”；选择继续后才能选择 AI 方案或输入人工结论。选择等待时，`review` 记录外部等待并仅阻塞关联工作单元；需求拆期应刷新来源后重新澄清，风险接受只在测试阶段处理。澄清确认后不允许修改决策结论；仅在外部条件已满足时允许使用 `aiw task decision resolve` 解除对应等待。
