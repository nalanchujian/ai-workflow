...
- `design-analysis`（可选）：通过已登录的 Chrome 原生复制指定 Figma 父节点，再按绿色容器、编号和留白裁成逻辑业务块图片，形成最小索引。
...
需求关联大型 Figma 节点时，先在 Chrome 中登录有权限的 Figma 账号并打开对应页签，再在创建任务时增加 `--design "<带 node-id 的 Figma 地址>"`。此时下一步先执行独立的 `design-analysis`；AIW 不使用或配置 Figma MCP，普通任务不受影响。
