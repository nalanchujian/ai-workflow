...
export class TaskFactGuard {
  ...
  async assertCommitted(input: { task: Task; paths: string[] }): Promise<void> {
    const paths = [...new Set(input.paths)];
    const uncommitted = await this.deps.repositoryStatus.uncommittedPaths({ projectRoot: input.task.repository, paths });
