...
export class MethodSourceResolver implements MethodSourceResolverPort {
  ...
  async resolve(source: MethodSource): Promise<ResolvedMethodSource> {
    ...
    assertMethodName(content.toString('utf8'), method[1]);
    return ResolvedMethodSourceSchema.parse({ ...source, revision: profile.revision, sha256: createHash('sha256').update(content).digest('hex') });
  }
