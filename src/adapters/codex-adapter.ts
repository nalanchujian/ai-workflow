...
function renderContext(request: RunRequest): string {
  ...
  const deliveryOutputContract = request.task.phase === 'implement' && acceptanceResultsPath !== undefined && testResultsPath !== undefined && deliveryPath !== undefined
    ? `\n当前是一个交付单元：必须在本次运行中完成代码实现、工程验证和验收测试，不能把验证或测试留给后续全局节点。\`artifacts/delivery.md\` 必须以 \`# 交付报告\` 开头，并固定包含 \`## 实际变更\`、\`## 工程验证\`、\`## 测试命令\`、\`## 测试结果\`、\`## 逐项验收\`、\`## 未完成事项与风险\`。\n\nAIW 会在 Codex 结束后**自行执行**已获批计划中的下列测试命令，并把原始 stdout/stderr、退出码写入 \`artifacts/test-results.yaml\` 和 \`runs/<run-id>/tests/\`。该文件由 AIW 生成，你不得创建或修改它：\n${testPlan}\n\n你仍应在 \`artifacts/delivery.md\` 的“测试命令”和“测试结果”中逐条写出测试 ID 与命令；只有你已实际运行且预期通过的命令才可写退出码 0，例如：\`TEST-LIST-01：pnpm test -- list-export；退出码：0。\` 若无法执行或结果未知，必须如实写明风险。\n\n同时生成 \`artifacts/acceptance-results.yaml\`，逐项声明**当前交付单元在 task.yaml 的 acceptanceRefs 中列出的验收项**，不得写入其他 AC：\n\`\`\`yaml\nschemaVersion: aiw.acceptance-results/v1\nitems:\n  - id: AC-01\n    status: passed # passed | failed | blocked | deferred | waived\n    evidence:\n      - artifacts/delivery.md\n    testResultRefs: [TEST-LIST-01]\n\`\`\`\n字段仅允许 \`id\`、\`status\`、\`evidence\`、\`testResultRefs\`；不得使用 \`acceptanceId\`、\`result\` 或 \`proofs\`。\`passed\` 必须引用上方某条由 AIW 执行、最终实际 \`status: passed\` 且 \`exitCode: 0\` 的测试记录；其他状态的验收项应使用空数组 \`testResultRefs: []\`。`
    : '';
  ...
  const factReferenceOverride = request.task.nodeId === 'clarify' && factRegisterPath !== undefined
    ? '\n正式事实引用规则优先于上文示例：`artifacts/acceptance.yaml` 的每个 item 必须包含 `factRefs`；`artifacts/decision-register.yaml` 的每个 item 也必须包含 `factRefs`。验收项允许字段为 `id`、`title`、`description`、`factRefs`；决策项允许字段为 `id`、`title`、`detail`、`type`、`factRefs`、`affects`、`status`、`options`、`recommendation`、`resolution`。'
    : '';
