...
function renderContext(request: RunRequest): string {
  ...
  const deliveryOutputContract = request.task.phase === 'implement' && acceptanceIntentPath !== undefined && acceptanceResultsPath !== undefined && testResultsPath !== undefined && deliveryPath !== undefined
    ? `\n当前是一个交付单元：必须在本次运行中完成代码实现、工程验证和验收测试，不能把验证或测试留给后续全局节点。交付报告必须先写入 \`${taskRoot}/${stagingPathFor(deliveryPath)}\`，并以 \`# 交付报告\` 开头，固定包含 \`## 实际变更\`、\`## 工程验证\`、\`## 测试计划\`、\`## 逐项验收\`、\`## 未完成事项与风险\`。\n\n在“测试计划”中逐条写出下列测试 ID 和原样命令，例如：\`TEST-LIST-01：pnpm test -- list-export\`。**不要**在交付报告、Handoff 或其他 Codex 产物中填写这些计划命令的退出码、通过/失败结论或模拟测试输出。AIW 会在 Codex 结束后作为唯一执行者运行下列已获批测试命令，并把原始 stdout/stderr、退出码和最终状态写入正式产物 \`${taskRoot}/${testResultsPath}\` 与 \`runs/<run-id>/tests/\`。该文件由 AIW 生成，你不得创建或修改它：\n${testPlan}\n\n同时生成 \`${taskRoot}/${stagingPathFor(acceptanceIntentPath)}\`，逐项声明**当前交付单元在 task.yaml 的 acceptanceRefs 中列出的验收意图**，不得写入其他 AC。evidence 必须引用发布后的正式路径 \`${deliveryPath}\`，不要引用暂存路径；\`testPlanRefs\` 必须引用上方测试 ID。**不得写入 status、passed、failed、blocked 或退出码。** AIW 会根据真实测试记录生成唯一正式结果 \`${taskRoot}/${acceptanceResultsPath}\`：\n\`\`\`yaml\nschemaVersion: aiw.acceptance-intent/v1\nitems:\n  - id: AC-01\n    evidence:\n      - ${deliveryPath}\n    testPlanRefs: [TEST-LIST-01]\n\`\`\`\n字段仅允许 \`id\`、\`evidence\`、\`testPlanRefs\`；不得使用 \`acceptanceId\`、\`status\`、\`result\` 或 \`proofs\`。`
    : '';
