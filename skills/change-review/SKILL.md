...
按 [references/output-template.md](references/output-template.md) 写入 `<tasks-root>/<task-id>/change-review.md`：四个一级区块依次写结果、评审产出、待确认项和下一步；通过上游 ID 和代码指纹引用既有结论，只记录最终差异、范围偏差、发现项与一致性证据。非需求未决项使用统一六列表，整份产物不超过三张表。完成后报告结果给编排器。
