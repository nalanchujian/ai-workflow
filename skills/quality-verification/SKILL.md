...
按 [references/output-template.md](references/output-template.md) 写入 `<tasks-root>/<task-id>/quality-verification.md`：四个一级区块依次写结果、验证产出、待确认项和下一步；通过 `AC-*`、`WP-*` 或轻量验收项引用上游，只记录验证结果、缺陷和代码身份。非需求未决项使用统一六列表，整份产物不超过三张表。完成后报告结果给编排器。
