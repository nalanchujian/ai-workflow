...
按 [references/output-template.md](references/output-template.md) 写入 `<tasks-root>/<task-id>/impact-analysis.md`：四个一级区块依次写结果、影响产出、待确认项和下一步；通过 `RF-*`、`AC-*` 引用上游，只新增影响项、调用链、回归范围及代码证据。非需求未决项使用统一六列表，整份产物不超过三张表。完成后报告结果给编排器。编排器创建本节点 pending gate，只有 `workflow-owner` 确认影响范围后才进入实施设计。
