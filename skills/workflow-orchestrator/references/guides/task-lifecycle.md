...
任务创建、节点执行和等待人工确认期间保持 `active`。每个节点完成后都创建人工 gate；节点无法继续时写为 `blocked`；变更评审 gate 批准且交付证据完整时写为 `completed`；明确取消时写为 `cancelled`。
...
`active` 只有三种合法形态：待启动时有 `next_node`；执行中有 `current_node` 且 `next_node=null`；等待人工确认时当前产物已登记并存在 gate，非终点 gate 同时保存批准后的 `next_node`，终点 gate 的 `next_node=null`。`start-node` 会消费 `next_node`，同一节点不能重复启动。
...
| `record-result` | `active` 执行中 | 当前节点匹配；产物身份、attempt、摘要有效；完成结果提供 gate owner | 完成时创建 pending gate；失败时进入 `blocked` |
| `request-gate` | `active` 已登记产物 | 旧任务当前产物已登记、尚未批准且下一节点正确 | 为当前节点补建 pending gate |
| `approve-gate` | `active` 等待 gate | `workflow-owner` actor；Owner 和精确确认指令匹配；当前产物未变化 | 非终点 gate 为 `approved`；终点 gate 将任务写为 `completed` |
| `reject-gate` | `active` 等待 gate | `workflow-owner` actor；精确拒绝指令匹配且提供原因 | 清除 gate，创建当前 blocker，任务为 `blocked` |
| `set-delivery` | `active` 开发实现中 | 开发产物为 `completed` | 写入四项代码身份 |
| `invalidate-from` | `active` 或 `blocked` | 目标有当前证据；失效范围覆盖全部 blocker 来源 | 删除范围内产物，保留 attempt 计数，恢复 `active` |
| `cancel` | `active` 或 `blocked` | 提供取消人和原因 | 清除 gate/blocker，任务为不可变 `cancelled` |
...
统一 gate owner 使用 `workflow-owner`，表示当前任务的人工决策者。需求受理与确认节点先整理 `RF-*` 事实和 `RQ-*` 疑点，再在执行阶段通过多轮对话按顺序处理每个 `RQ-*`：Owner 确认或修改后才进入下一项，未处理时保持当前项。全部疑点均形成状态为“已人工确认”的 `CL-*` 后，CLI 才允许登记节点一完成产物并创建 Gate；该 Gate 确认整份需求定义准确。编排器不得代替用户确认疑点、批准 Gate、补写或推导确认文本；“继续”“执行下一节点”等自然语言不具备确认效力。
