...
CLI 在待启动节点的写入结果和 `audit` 输出中返回 `start_confirmation`，固定格式为 `执行节点：<task-id>/<node-id>@<revision>`；`start-node` 必须收到完全匹配的 `--confirmation`。CLI 在 pending Gate 的写入结果和 `audit` 输出中返回 `approve_confirmation`、`reject_confirmation`，批准格式固定为 `确认节点：<task-id>/<node-id>@<revision>`，拒绝格式固定为 `拒绝节点：<task-id>/<node-id>@<revision>`。调用方必须原样展示这些指令，不能自行生成近似文本；revision 变化后旧指令立即失效。
