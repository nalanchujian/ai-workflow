...
按 [references/output-template.md](references/output-template.md) 写入 `<tasks-root>/<task-id>/implementation-plan.md`：结果写方案结论和规模，交付按工作包组织方案边界、文件变更、行为契约、验证与回滚，轻量需求增加“最小验收条件”；未决项写风险和 blocker；交接写开发准入、授权含义和 Gate 交接。完成后报告结果给编排器。
