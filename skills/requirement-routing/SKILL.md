...
用户必须提供任务目录。始终读取 `task.yaml` 和已批准的 `requirement-analysis.md`。该产物已经包含需求范围、事实以及 Owner 逐条确认形成的决策。从 `task.yaml.target_module` 和需求事实出发，只扫描路径分流所需的边界证据。
...
需求分流不在产物中维护人工确认字段。`unresolved_items` 只记录尚未关闭的风险、缺陷、资料依赖或环境阻塞，字段语义与其他非需求节点一致。分流成功后由编排器创建本节点 pending gate，`workflow-owner` 确认路径后才进入对应后续节点。需要补充或重新确认需求时，由编排器根据阻塞结果退回需求受理与确认。

## 输出

写入 `<tasks-root>/<task-id>/requirement-routing.json`。新产物统一使用产物 Schema 版本 1：
...
`reason` 只保存一句话结论；`evidence` 每项保存一条可核验的需求事实、代码位置或边界信号，不复制需求定义原文，也不写实施方案。报告路径、结果状态和分流原因，由编排器根据路径契约推导后续节点、创建人工 gate 并写入 `task.yaml`。聊天中仅说明判定结果、关键证据、产物路径和确认请求。

`status=completed` 时 `path` 必须是已注册路径且 `unresolved_items=[]`；`status=blocked` 时 `path` 必须为 `null`，并至少提供一个未决项。每个未决项固定包含 `id`、`type`、`issue_and_impact`、`owner`、`retry_node`、`completion_condition`。后续节点从 [工作流路径](../workflow-orchestrator/references/contracts/workflow-paths.json) 推导，产物文件名从节点注册表推导。
